...
@RestController
public class UpdatePostTotalLike {
    private static final Logger logger = LoggerFactory.getLogger(UpdatePostTotalLike.class);
    ...
    public ResponseEntity UpdatePostTotalLike() {
        ...
        Set<String> objectIds = redisService.GetAllSetMembers(UPDATE+POST_LIKE);
        if (objectIds.isEmpty()) {
            logger.info("objectIds is empty");
        } else {
            List<Long> postIds = new ArrayList<>();
            for (String objectId : objectIds) {
                Map<String, String> hashSetMap = redisService.GetHashValue(UPDATE+POST_LIKE + ":::" + objectId);
                hashSetMap.entrySet().stream().forEach(entry -> {
                    Long postId = Long.valueOf(objectId);
                    String userId = entry.getKey();
                    postIds.add(postId);
                    redisService.DeleteMember(UPDATE+POST_LIKE + ":::" + objectId, userId);
                    if (redisService.NumOfMembers(objectId) == 0) {
                        redisService.RemoveHashSet(UPDATE+POST_LIKE, objectId);
                        logger.info("Removed hash set: {}", UPDATE+POST_LIKE);
                    } else {
                        //
                    }
                });
            ...
            postInfoService.CalculatePostTotalLike(postIds);
        }
