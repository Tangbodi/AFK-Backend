...
public class UpdateCommentLikeController {
    ...
    public ResponseEntity UpdateCommentLike() {
        ...
        Set<String> objectIds = redisService.GetAllSetMembers(COMMENT_LIKE);
        if (objectIds.isEmpty()) {
            ...
            for (String objectId : objectIds) {
                ...
                Map<String, String> hashSetMap = redisService.GetHashValue(COMMENT_LIKE + ":::" + objectId);
                hashSetMap.entrySet().stream().forEach(entry -> {
                    ...
                    redisService.DeleteMember(COMMENT_LIKE + ":::" + objectId, userId);
                    if (redisService.NumOfMembers(objectId) == 0) {
                        redisService.RemoveHashSet(COMMENT_LIKE, objectId);
