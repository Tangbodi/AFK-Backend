...
public interface PostGameMapRepository extends JpaRepository<PostsGamesMap, Long> {
    ...
    @Query(value = "SELECT p.post_id, g.game_name, ui.user_id, ui.username, p.title,p.text_render, pi.view, pi.comment_reply, pi.like, pi.save, ifnull(ufp.like_status,0) AS like_status, ifnull(ufp.save_status,0) AS save_status, p.created_at\n" +
            "FROM afk.posts_games_map pgm\n" +
            ...
            "JOIN afk.posts_info pi ON pi.post_id = pgm.post_id\n" +
            "JOIN afk.users_favorite_posts ufp ON pi.post_id = ufp.post_id\n" +
            "LEFT JOIN afk.games g ON g.game_id = pgm.game_id\n" +
            "WHERE pgm.genre_id = :genreId\n" +
            "AND pgm.game_id = :gameId\n" +
            "AND pgm.post_id = :postId\n" +
            "AND ufp.user_id = :userId", nativeQuery = true)
    List<Map<String, Object>> findByGenreGamePostId(@Param("genreId") Byte genreId, @Param("gameId") Short gameId, @Param("postId") Long postId, @Param("userId") Long userId);
}
