...
public interface CommentOnPostMentionRepository extends JpaRepository<CommentOnPostMention,Long> {
    ...
    @Query(value = "UPDATE afk.comment_on_post_mentions SET mention_on = :status WHERE user_id = :userId AND post_id = :postId", nativeQuery = true)
    List<Map<Short, Object>> updateSettingByUserId(@Param("userId") Long userId);
    @Query(value = "SELECT copm.user_id, copm.mention_on AS comment_on_post, locm.mention_on AS like_on_comment, lopm.mention_on AS like_on_post, posgm.mention_on AS post_on_saved_game, rocm.mention_on AS reply_on_comment, sopm.mention_on AS save_on_post\n" +
            ...
            "WHERE copm.user_id IN (:userIds)", nativeQuery = true)
    List<Map<Short, Object>> findSettingByUserIds(@Param("userIds") List<Long> userIds);
